# LoggerLocalConnectors

The logger flow creates a log message entry in a specific log type as provided via input parameters. The Local Connectors version of the Logger logs to either a Log file, a CSV file or a Database table. See **Notes** below regarding the use of the Logger flows.
...
1. Create a new flow called **PADFramework: LoggerLocalConnectors** - make sure to not enable Power Fx when creating it
...
For example, if you do not plan to ever log to a Database, you do not need to create the database and the stored procedures, and then simply skip the **LogToDatabase** subflow. Then delete the call to this subflow from **Main** after creating it.
...
This flow only supports logging to CSV files, Log files and Databases as these three types do not require setting up connection references to the cloud and as such, do not require Premium. They also do not require any connections to exist in the environment, so, it will usually be easier to import to some environments, even if the user has Premium. Other types of logging (SharePoint lists and Dataverse tables are in a separate flow called **LoggerCloudConnectors**).
