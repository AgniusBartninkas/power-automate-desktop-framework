...
BEGIN
    ...
    BEGIN TRY
        ...
        EXEC @Result = sp_getapplock @Resource = 'usp_CompleteWorkItem', @LockMode = 'Exclusive', @LockTimeout = 30000, @LockOwner = 'Transaction'

        IF @Result < 0
        BEGIN
            SET @Status = -2
            SET @Message = N'Failed to lock stored procedure usp_CompleteWorkItem exclusively'
        ...
        EXEC [dbo].[usp_InsertWorkItemResult] @WorkItemId, @MachineName, @ProjectNumber, @FlowName, @WorkItemStatus, @WorkItemReason, @WorkItemMessage, @ProcessingStartUTC

        -- lock and machine id is removed by usp_InsertWorkItemResult
        UPDATE [WorkItem]
        SET
            [StatusId] = @StatusId,
            [Processed] = 1,
        ...
        SET @Status = 0
        SET @Message = N'Successfully completed work item with id ' + CAST(@WorkItemId AS NVARCHAR)
